...
(deftest test-request-dispatch
  ...
  (is (nil? (request "/whatiscollected/1.0/lg=11&lt=2.2" routes)) "longitude missing decimal")
  (is (nil? (request "/whatiscollected/1.0/lg=1.1&lt=22" routes)) "latitude missing decimal")
  (is (nil? (request "/other/1.0/lg=1.1&lt=2.2" routes)) "api suffix missing")
  (is (nil? (request "/whatiscollected/x.x/lg=1.1&lt=2.2" routes)) "version missing")
  (is (nil? (request "/whatiscollected/1.0/lg=1.1&lt=2.2&y=a&m=1&d=1" routes)) "invalid year")
  (is (nil? (request "/whatiscollected/1.0/lg=1.1&lt=2.2&y=2001&m=a&d=1" routes)) "invalid month")
  (is (nil? (request "/whatiscollected/1.0/lg=1.1&lt=2.2&y=2001&m=1&d=a" routes)) "invalid day")
  (with-redefs [garbageday.model/collection-schedule #(if (= [%1 %2] ["1.1" "2.2"]) "Tuesday 2")]
    (is (= 200 (:status (request "/whatiscollected/1.0/lg=1.1&lt=2.2&y=2012&m=1&d=1" routes)))
